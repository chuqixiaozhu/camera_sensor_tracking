...
if {0 < $argc} {
    ...
    set opt(alpha_metric) [lindex $argv 0]
...
proc dispatching {time_stamp} {
    global opt mnodes moving_sensors subregions level2_index nodes_num
    # Stop moving sensors
    for {set k 0} {$k < $opt(ntarget)} {incr k} {
    ...
    for {set k 0} {$k < $opt(ntarget)} {incr k} {
        set num [llength $moving_sensors($k)]
        incr nodes_num($k) $num
    }
}
...
proc average_emt {} {
    global EMT opt nodes_num
    set sum 0
    for {set i 0} {$i < $opt(ntarget)} {incr i} {
        #incr sum $EMT($i)
        incr sum $nodes_num($i)
        #puts "EMT($i): $EMT($i)"; # test
    }
    #set opt(AVG_EMT) [expr 1.0 * $sum / $opt(ntarget)]
    set opt(AVG_NODES) [expr 1.0 * $sum / $opt(ntarget)]
}
