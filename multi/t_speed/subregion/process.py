...
for i in range(count):
    for t_speed in range(1, 7):
        subprocess.call(['ns', 'camera.tcl', str(t_speed), result_file])

# Process the results
rf = open(result_file, 'r')
emt_file = open('t_speed-subregion_vs_emt', 'w')
tmd_file = open('t_speed-subregion_vs_tmd', 'w')
emt = dict()
tmd = dict()
for line in rf:
    results = line.split()
