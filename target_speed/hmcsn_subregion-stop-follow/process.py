...
for i in range(count):
    for target_speed in range(1, 7):
        subprocess.call(['ns', 'camera.tcl', str(target_speed), result_file])

# Process the results
rf = open(result_file, 'r')
emt_file = open('t_speed-grid_vs_emt', 'w')
tmd_file = open('t_speed-grid_vs_tmd', 'w')
emt = dict()
tmd = dict()
for line in rf:
    results = line.split()
