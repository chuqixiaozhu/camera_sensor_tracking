...
set opt(obstacle_number) 2;                     # number of obstacles
set opt(obstacle_length) 30;        # Length of a Square Obstacle

source $opt(normal)
if {0 < $argc} {
    #set opt(nfnode) [lindex $argv 0]
    set opt(nmnode) [lindex $argv 0]
    #set opt(obstacle_number) [lindex $argv 0]
...
proc gridding {target_ time_stamp} {
    ...
    set target_x [$target set X_]
    set target_y [$target set Y_]
    puts "@562 in gridding: target ($target_x, $target_y)"; # test
...
proc dispatching {target_ time_stamp} {
    ...
    puts "@709 dest_x = $dest_x, dest_y = $dest_y"; # test
...
# A Movement of this Target
        ...
        #create_obstacles dest_x dest_y
        ...
        puts "@882 Target from ($target_lx, $target_ly) to ($dest_x, $dest_y)"; # test
